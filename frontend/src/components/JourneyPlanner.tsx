...
export default function JourneyPlanner(){
    ...
    const handleSearch = async () => {
        ...
        // call API
        try {
            const response = await axios.get<RouteData>("http://localhost:8080/api/journey/route", {
                params: {
                    start: start,
                    end: end,
        ...
        <div className='container mx-auto p-4 max-w-4xl'>
            ...
            <div className='card bg-base-100 shadow-xl mb-8'>
                <div className='card-body'>
                    <h2 className='card-title text-2xl text-neutral mb-4'>Plan Your Journey</h2>
                    ...
                    <form action={handleSearch} className='flex flex-col gap-4'>
                        ...
                        <div className="flex gap-6 py-2">
                            <div className="form-control">
                                ...
                                />
                                <span className="label-text font-bold">Fastest Time</span> 
                                </label>
                            </div>
                            ...
                            <div className="form-control">
                                ...
                                <input 
                                    ...
                                    onChange={() => setOptimisedRoute(false)}
                                />
                                <span className="label-text font-bold">Fewest Changes</span> 
                                </label>
                            </div>
                        </div>

                        <button type='submit' className='btn btn-primary w-full' disabled={isLoading}>
                            {isLoading ? <span className="loading loading-spinner"></span> : "Find Route"}
                        </button>
                    </form>
                </div>
            </div>
                ...
                <div className='card bg-base-100 shadow-xl'>
                    <div className='card-body'>
                        <div className='flex justify-between items-center border-b pb-4 mb-4'>
                            <div>
                                <h2 className='text-3xl font-bold text-secondary'>{routeData.totalTime} Mins</h2>
                                <p className='text-gray-500'>Total Journey Time</p>
                            </div>
                            <div className='text-right'>
                                <h2 className='text-3xl font-bold'>{routeData.totalChanges}</h2>
                        ...
                        <ul className='steps steps-vertical'>
                            {routeData.path.map((step, index) => {
                                ...
                                const stepColor = getStepColorClass(step.line);
                                ...
                                return (
                                    ...
                                    >
                                        <div className='text-left ml-4'>
                                            <p className='font-bold text-lg'>{step.station}</p>
                                            <p className='text-sm opacity-70'>
                                                {index === 0 ? "Start Journey" : (isWalking ? "Walk" : `Board ${step.line} line`)}
                                            </p>
                                        </div>
                                    </li>
                                )
